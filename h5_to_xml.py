...
def build_dataset(img_folder, mat_file_name, h5_name):
    boundingboxes = boundingboxes_constructor( mat_file_name)
...
def to_xml(rows):
    ...
    with open('annotation/' + rows.iloc[0]['image_name'].strip('.png') + '.xml', 'w') as f:
        f.write('\n'.join(xml))
...
if __name__ == '__main__':
    ...
    dataframe = pd.read_hdf(h5_file_path, 'table') 
